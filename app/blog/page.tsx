        ...
        <div className="container-custom relative z-10">
          ...
          >
            <motion.div
              ...
              className="inline-block px-6 py-2 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-full mb-6"
            >
            ...
            >
              ...
              <input
                ...
                className="w-full pl-16 pr-6 py-5 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20 transition-all duration-300 text-lg"
              />
            </motion.div>
          </motion.div>
        </div>
        ...
        <section className="py-16 relative">
          <div className="container-custom">
            ...
            >
              ...
              <div className="h-1 w-32 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full" />
            </motion.div>
              ...
              {postsDestaque.map((post, index) => (
                ...
                >
                  <Link href={`/blog/${post.slug}`}>
                    <div className="relative bg-slate-800/50 backdrop-blur-xl rounded-2xl overflow-hidden border border-slate-700/50 hover:border-cyan-500/50 transition-all duration-300 hover:shadow-premium">
                      {/* Badge Destaque */}
                      <div className="absolute top-4 left-4 z-10 px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-full">
                        <span className="text-white font-bold text-xs tracking-wider">DESTAQUE</span>
                      </div>

                      {/* Imagem */}
                      <div className="relative h-56 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 overflow-hidden">
          ...
          {postsFiltrados.length > 0 ? (
            <div className="grid md:grid-cols-2 gap-8">
              {postsFiltrados.map((post, index) => (
                ...
                >
                  <Link href={`/blog/${post.slug}`}>
                    <div className="relative bg-slate-800/30 backdrop-blur-xl rounded-2xl overflow-hidden border border-slate-700/30 hover:border-cyan-500/50 transition-all duration-300 hover:shadow-premium p-6">
                      <div className="flex items-start gap-4 mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-3">
                            <span className="px-3 py-1 bg-slate-700/50 border border-slate-600/50 rounded-full text-slate-300 text-xs font-semibold">
                              {post.categoria}
                            </span>
                            <div className="flex items-center gap-2 text-slate-400 text-sm">
                              <Calendar className="w-4 h-4" />
      ...
      <section className="py-20 relative">
        <div className="container-custom">
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="relative bg-gradient-to-r from-cyan-500/10 to-blue-500/10 backdrop-blur-xl rounded-3xl p-12 border border-cyan-500/20 text-center overflow-hidden"
          >
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
              className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-full blur-3xl"
            />
            ...
            <Link
              href="/diagnostico"
              className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold rounded-xl shadow-glow hover:shadow-futuristic transition-all duration-300 hover:scale-105 relative z-10"
            >
